package banger.framework.sql.util;
...
import banger.framework.sql.cache.CacheKey;
import banger.framework.sql.cache.CacheModel;
import banger.framework.sql.command.IDataReader;
import banger.framework.sql.driver.IDataProvider;
import banger.framework.sql.event.SqlExecuteEvent;
import banger.framework.sql.mapping.ISqlMap;
import banger.framework.sql.mapping.SqlItem;
import banger.framework.sql.mapping.SqlLink;
import banger.framework.sql.mapping.SqlLink.LinkColumn;
import banger.framework.sql.mapping.SqlLinkGroup;
import banger.framework.util.MathUtil;
...
//        System.out.println(sqlString.toString());
			...
			for(SqlLinkGroup linkGroup : sqlItem.getLinkGroups()){
				...
				for(SqlLink link : linkGroup.getLinks()) {
					if (!table.contains(link.getPkey()))
						throw new RuntimeException(StringUtil.format("在 sql {0} 的返回结果中,找不到 link列{1}", sqlId, link.getPkey()));
				}
	...
	private void linkTable(DataTable table,SqlLink link,DataTable linkTable,String sqlId){
		...
		if(!linkTable.contains(fkey))throw new RuntimeException(StringUtil.format("在 sql {0} 的返回结果中,找不到 列{1}",link.getSqlId(),fkey));
		Set<String> addCols = new HashSet<String>();
		for(LinkColumn col : link.getColumns()){
