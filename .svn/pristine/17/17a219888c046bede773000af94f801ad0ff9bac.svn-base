...
import banger.framework.EntityMapException;
import banger.framework.sql.mapping.SqlMap;
...
public class EntityMap implements IEntityMap,InitializingBean {
	...
	public boolean containKey(Class<?> type){
		if(filterSet.contains(type))return false;
		Class<?> cls = type;
		while(cls!=null) {
	...
	static class ClassMapLoader {
		...
		private static Entity buildEntity(XmlNode node,String filename){
			if(node.getName().equalsIgnoreCase("class")){
				if(!node.getAttributes().containsKey("name")) throw new EntityMapException("class节点 name属性未定义 文件 "+filename);
				if(!node.getAttributes().containsKey("table")) throw new EntityMapException("class节点 table属性未定义 文件 "+filename);
				...
				for(XmlNode n : node.getChildNodes()){
					...
					if(prop!=null){
						e.addProperty(prop);
						if(prop instanceof Id)e.id = (Id)prop;
					}
				}
				return e;
			}
		...
		private static Property buildProperty(XmlNode node,String filename){
			if(node.getName().equalsIgnoreCase("property") || node.getName().equalsIgnoreCase("id")){
				if(!node.getAttributes().containsKey("name")) throw new EntityMapException("property节点 name属性未定义 文件 "+filename);
				if(!node.getAttributes().containsKey("type")) throw new EntityMapException("property节点 type属性未定义 文件 "+filename);
				String name = node.getAttributes().get("name").getValue();
		...
		private static Column buildColumn(XmlNode node,String filename){
			if(node.getName().equalsIgnoreCase("column")){
				if(!node.getAttributes().containsKey("name")) throw new EntityMapException("column节点 name属性未定义 文件 "+filename);
				String name = node.getAttributes().get("name").getValue();
