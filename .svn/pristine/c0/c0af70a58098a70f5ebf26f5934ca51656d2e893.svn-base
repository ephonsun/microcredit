	...
	Δ.ctrl.window=δ={
		...
		,dock:function(ctrl,styles){
			...
			this.buildImageElement=function(imgIndex,boxIndex){				//构造停靠标记图片
				...
				var imageObj = Δ.widget.graph.getElement(imgSrc,"","<div toward=\""+imgSrc.substr(0,imgSrc.length-4)+"\" >");
				with(this.styles){
					imageObj.attr("class",dockIconStyle);
		...
		,control:function(){
			...
			this.buildButtonElement=function(index,type,hiddenFlag){
				...
				var icon = Δ.widget.graph.getElement(type,"normal","<input type=button >");
				icon.attr(i.attr).css(i.css);
