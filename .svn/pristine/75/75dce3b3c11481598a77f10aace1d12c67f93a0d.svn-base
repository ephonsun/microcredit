...
function deleteVideoCancel(queueID){
...
function setVideoUpload(script,sizeLimit,queueSizeLimit,uploadFileType) {
	...
	var existQueueID = '';
	$("#fileVideoInput").uploadify({
		/*注意前面需要书写path的代码*/
		'uploader' : '../uploadify/uploadify.swf',
		'script' : script,
		'cancelImg' : '../uploadify/cancel.png',
		'queueID' : 'fileVideoQueue', //和存放队列的DIV的id一致 
		'fileDataName' : 'fileVideoInput', //和以下input的name属性一致 
		'auto' : true, //是否自动开始
		'multi' : false, //是否支持多文件上传 
		//'hideButton'     : true, //隐藏浏览按钮的图片
		'buttonText' : escape('上传视频'),//浏览按钮上的文字  
		...
		'onSelect':function(event, queueID, fileObj){ //加入队列时候上传重名的文件不会触发这个函数
			...
			if($.inArray(type.toLowerCase(), typeArrs) < 0){
				...
				setTimeout("$('#fileVideoInput').uploadifyCancel('" + queueID + "')", 2000);
				deleteVideoCancel(queueID);
		...
		'onComplete' : function(event, queueID, fileObj, response, data) {
			if(erroTypeName.length >0 || moreSizeName.length > 0 || uploadedRepeatFiles.length > 0) {
                erroTypeName = []; //清空
                moreSizeName = []; //清空
                uploadedRepeatFiles = []; //清空
			}else{
                var existVideo = $('.videoFileClass');
                if (existVideo.length > 0) {
                    ...
                    deleteFileId.setAttribute("value", $('.videoFileClass').attr("id"));
                ...
                fileInputSize.setAttribute("name", "fileSizeVideos");
                ...
                fileInputOldName.setAttribute("name", "fileNameOldVideos");
		...
		'onSelectOnce':function(event, data){
			...
			if(queueFullBool == false){
				if(erroTypeName.length > 0 && onSelectBool == true){
					showConfirm({ icon:'warning', content:'文件：<br />' + erroTypeName.join('<br />') + '<br />格式错误，目前只支持mp4、avi视频文件格式上传！'});
