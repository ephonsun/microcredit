...
function query(){
    ...
    jQuery.ajax({
        type: 'post',
        dataType: 'json',
        url: '../intoTemplatesInfo/queryIntoTemplatesInfoColumnList.html',
        data: {
        ...
        success: function (data) {
            ...
            if (data.rows) {
                ...
                if (!isOld) {
                    ...
                    var isMustArr=oldJson["isMusts"].split(",");
...
function changeValue(templateId){
    ...
    if(isOld){
        ...
        $(".radio").each(function () {
            isMusts.push($(this).attr("checked")?1:0);
        });
        $(".ui-text-text").each(function(){
            //如果不输入，或者为空
            if(!$(this).val()){
...
function save(){
    ...
    jQuery.ajax({
        type : 'post',
        url : '../intoTemplatesInfo/saveIntoTemplate.html',
        data : {
            "jsons":datas
        },
...
function closeDialog(){
    var dialog = getDialog('getModelTemplateInfoSetPage');
    var win = tabs.getIframeWindow("intoPiecesInfo");
