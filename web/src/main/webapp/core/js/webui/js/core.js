...
var nff = {version:"0.9.2",author:"zhusw","Δδ":"得尔塔"};
...
(function($,Δ){
		...
		if (object!=null && source!=null){
			...
			for(s in source){
				...
				switch(t){
					...
					break;
					case "boolean" : object[nm] = (uv=="TRUE" || uv=="READONLY" || uv=="CHECKED") ? true : false;
					break;
	...
	/* --------------------------------------------动态样式----------------------------------------------  */
	Δ.dynamic={
		style:function(n,o,a,s,i){
			this.normal =this.active =this.over =this.interval =this.select="";
			this.obj =null;
			if (n) this.normal =n;
			if (o) this.over =o;
			if (a) this.active =a;
	...
	Δ.format=function(val,format){	
		switch(typeof val){
			...
			case "object":
				if(val instanceof Date)return Δ.dateFormat(val,format);
		}
	};
	/* 数据格式化 */
	...
	/* 日期格式化 */
	Δ.dateFormat=function(date,format){
		...
		var dateStr = format.replace(yearMarker,year).replace(monthMarker,month).replace(dayMarker,day);
		return dateStr;
	};
