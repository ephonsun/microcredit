	...
	Δ.ctrl.forms=δ={
		...
		,cell:function(ctrl,row){
			...
			this.getWidth=function(){
				...
				switch(this.type){
					case "head":
						return this.ctrl.labelWidthRatio*rate;
					case "content":
						if(this.item.title){
							return (cols-this.ctrl.labelWidthRatio)*rate;
						}else{
							return cols*rate;
						}
					case "place":
		...
		,control:function(){
			...
			this.stuffAllRow=function(){
				for(var i=0;i<this.rows.length;i++){
					this.rows[i].stuffCell();
				}
			};
			this.clear=function(){
				this.rows=[];
		...
		,fn:{
			create:function(context){
				...
				for(var i=0;i<context.children.length;i++){
					...
					if(!setting.nullable)setting["nullable"] = false;
