	...
	Δ.ctrl.dao=δ={					//控件数据访问接口类
		...
		,datatree:function(){
			...
			this.getNode=function(indexs){
				var tree = (this.root)?this.root:eval(this.dataSourceName);
				if(tree){
			...
			this.getTree=function(){
				var tree = (this.root)?this.root:eval(this.dataSourceName);
				if(tree){
					if(this.parentField!="" && tree.constructor==Array){
						if(tree.length>0 && !tree[0].children){
							...
							for(var i=0;i<tree.length;i++){
								if(!tree[i].isLeaf)newTree.push(tree[i]);
							}
							Δ.setJson(this.dataSourceName,newTree);
							return newTree;
						}
					}
					return tree;
		...
		,fn:{
			...
			,createPropertyTree:function(ctx){
				var propertytree = new δ.propertytree();
				with(propertytree){
					if(ctx.table)dataSourceName=ctx.table;
					if(ctx.id)valueField=ctx.id;
					if(ctx.name)textField=ctx.name;
					if(ctx.pid)parentField=ctx.pid;
					if(ctx.catalog)catalogField=ctx.catalog;
					if(ctx.group)groupField=ctx.group;
				}
				return propertytree;
			}
