			...
			,createControl:function(ctrl,node,setting){
				...
				switch(setting.editor){
					...
					case "textbox":
						...
						editor.events.onLostFocus=function(args){
					...
					case "items":
						...
						editor.events.onOk=function(args){
	...
	Δ.ctrl.itemsbox=δ={
		...
		,fn:{
			...
			,addEvents:function(ctrl){
				...
				ctrl.ok.events.onClick=function(){
					δ.fn.okClick(ctrl);
				};
				ctrl.cancel.events.onClick=function(){
			...
			,okClick:function(ctrl){
				if(ctrl.events.onOk){
					Δ.raise(ctrl.events.onOk,{"ctrl":this,"data":ctrl.grid.datatable.table});
				}
				ctrl.panel.hide();
			}
