...
function upLoadFile(){
    ...
    if(filename!=""){
		judge(2);
        ajaxFileUpload({
            url:"../sysUpgrade/doUpload.html?fullFilename="+encodeURI(encodeURI(filename))+"&random="+Math.random()*10000,
            id:'file',
            callback:function(a){
...
function setupConfirm(){
    var serviceFilename = $("#saveFilename").val();
    var servicePath = $("#savePath").val();
    var postJson = {"saveFilename":serviceFilename,"savePath":servicePath};
    jQuery.post("../sysUpgrade/confirmUpgrade.html?random="+Math.random()*1000000,postJson,
        function(text){
            if(text=="success"){
               judge(4);
            }
