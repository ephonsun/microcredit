$(function(){
    ...
    $('#importUser').bind("click",function(){
        importUser();
        //执行完解除绑定click事件
        $('#importUser').unbind('click');
    });
})
/*下载导入机构模板*/
function downloadDeptExcel(){
    window.location="../user/downloadExcelFile.html";
}
function failMessage(){
    var filename = $("#filename").val();
    if(filename){
        window.location="../user/downloadFailFile.html?filename="+encodeURI(encodeURI(filename))+"&random="+Math.random()*10000;
    }
}
/*得到机构导入结果信息*/
...
function upLoadFile(){
    ...
    if(filename!=""){
        ajaxFileUpload({
            url:"../user/doUpload.html?fullFilename="+encodeURI(encodeURI(filename))+"&random="+Math.random()*10000,
            id:'file',
...
function importUser(){
    ...
    jQuery.ajax({
        type : 'post',
        url : '../user/doImportUser.html?saveFilename='+encodeURI(encodeURI(saveFilename)),
        data:postJson,
        success : function(jo) {
          /*  showConfirm({
                icon: 'succeed',
...
function loadExcelHead(filename){
	jQuery.post("../user/getExcelHeadData.html?random="+Math.random()*1000000,{"saveFilename":filename},
        function(html){
			if(html=="error"){
